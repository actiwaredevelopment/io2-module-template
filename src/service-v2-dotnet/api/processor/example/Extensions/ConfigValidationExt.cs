namespace Module.IOTemplate.Api.Processor.Example.Extensions
{
    ...
    public static class ConfigValidationExt
    {
        ...
        public static bool Validate(this Development.SDK.Module.Data.Common.ItemConfig config, Development.SDK.Module.Controller.Helper? helper)
        {
            ...
            {
                helper?.WriteMessage(Development.SDK.Module.Enums.ReportLevel.Error, "LOG_VALIDATE_ERROR_NO_LOGIN_PROFILE_SELECTED", "Failed Validation: No login profile was set in the configuration for the process node. Please check if a login profile was entered and if it still exists.");
                return false;
            }
